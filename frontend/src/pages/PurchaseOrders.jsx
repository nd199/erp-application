...
import { FiPackage, FiPlus, FiTrash2, FiExternalLink } from 'react-icons/fi'
...
import { fetchPurchaseOrders, createPurchaseOrder, updatePurchaseOrderStatus, deletePurchaseOrder } from '../store/purchaseOrderThunks'
...
function PurchaseOrders() {
  ...
  const { orders, loading, total } = useSelector((s) => s.purchaseOrders)
  ...
  useEffect(() => { dispatch(fetchProducts()); dispatch(fetchSuppliers({ size: 100 })) }, [dispatch])
  ...
  const handleDelete = async () => {
    try {
      await dispatch(deletePurchaseOrder(selected.id)).unwrap()
      ...
      key: 'id', label: 'Order', render: (val, row) => (
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-violet-500/15 to-purple-500/10 border border-white/[0.06] flex items-center justify-center shrink-0">
            <FiPackage className="w-4 h-4 text-violet-400" />
          </div>
          <div>
            <p className="text-white font-semibold text-sm">#{val}</p>
            <p className="text-[11px] text-gray-500">{row.supplierName || 'Supplier ' + row.supplierId}</p>
          </div>
        </div>
      ),
    },
  ...
  return (
    <div>
      <PageHeader title="Purchase Orders" subtitle="Manage supplier purchase orders" icon={FiPackage} actionLabel="New Order" onAction={openCreate} actionIcon={FiPlus} />
      ...
      {loading && orders.length === 0 ? <LoadingSpinner /> : total === 0 && !query && !status ? (
        <EmptyState title="No purchase orders found" description="Create your first purchase order." />
      ) : (
        <DataTable
          columns={columns}
          data={orders}
      ...
      {/* Create Order Modal */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="New Purchase Order" subtitle="Add items to create an order." size="lg">
        <Formik initialValues={{ notes: '' }} validationSchema={createSchema} onSubmit={handleCreateOrder}>
          {({ isSubmitting }) => (
            <Form className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1 block">Supplier</label>
                <select value={supplierId} onChange={(e) => setSupplierId(e.target.value)} className="w-full bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] rounded-xl px-3 py-2.5 text-sm text-gray-300 outline-none">
                  <option value="">Select supplier</option>
                  {suppliersList.filter((s) => s.active).map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select>
              </div>
              ...
              <div className="space-y-3">
                ...
                <button type="button" onClick={() => setOrderItems([...orderItems, { productId: '', quantity: 1, unitPrice: '' }])} className="flex items-center gap-2 text-xs text-violet-400 hover:text-violet-300 transition-colors cursor-pointer">
      ...
      {/* Detail Modal */}
      <Modal isOpen={detailOpen} onClose={() => setDetailOpen(false)} title={`Purchase Order #${selected?.id || ''}`} subtitle="Order details and items" size="lg">
        {selected && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <p className="text-[11px] text-gray-500 uppercase tracking-wider mb-1">Supplier</p>
                <p className="text-sm text-gray-300">{selected.supplierName || 'Supplier ' + selected.supplierId}</p>
              </div>
            </div>
            ...
            {selected.status === 'PENDING' && (
              <div>
                ...
                <div className="flex gap-2">
                  <button onClick={() => { handleStatusChange(selected.id, 'RECEIVED'); setDetailOpen(false) }} className="px-3 py-1.5 text-xs font-medium text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 rounded-lg hover:bg-emerald-500/20 transition-all cursor-pointer">Mark Received</button>
      ...
      <ConfirmModal isOpen={confirmOpen} onClose={() => setConfirmOpen(false)} onConfirm={handleDelete} title="Delete Order" message={`Delete purchase order #${selected?.id}? This action cannot be undone.`} />
    </div>
  )
}
